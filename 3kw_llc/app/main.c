...
uint8_t u8ResetEnabled = 0;
uint8_t open_llc = 0;
uint8_t fault_state;
uint8_t delay_state;
uint32_t comp_state;
DemoModeTypeDef DemoState;
static uint8_t fg_OV = 0;
static uint8_t fg_OC = 0;
uint32_t isrflags;
uint32_t cr1its ;
static int8_t debounceOV;
static int8_t debounceOC;
static int8_t debounceOV2;
static int8_t debounceOC2;
uint32_t Vout;
uint32_t Ia;
...
{
    /* 1.Initialize the ST hal driver */
    HAL_Init();

    /* 2.Configure the system clock with HSI to have a system clock = 64 Mhz */
    SystemClock_Config();

    /* 3.Initialize UART for communication with Master(PC)*/
    UART_Config();
    
    /* 4.Initialize DAC1_CH2 for over-current threshold on COMP6 */
    DAC1_CH2_Config();

    /* 5.Initialize COMP2 for over-current protection */
    COMP2_Config();

    /* 6.Initialize HRTIM and related inputs */
    HRTIM_Config();

    /* 7.Initialize ADC to be triggered by the HRTIMER */
    ADC_Config();

    /* 8.Initialize outputs (it has to be done after HRTIM init) */
    GPIO_LLC_HB_outputs_Config();

    /* 9.Initialize LLC control parameters*/
    llc_cal.Vref =LLC_VOUT_REF;
    llc_cal.Vref_cmd = LLC_VOUT_CMD;
    llc_cal.flag_ssOver = 0;
    llc_cal.B0 = LLC_B0;//0.5;
    llc_cal.B1 = LLC_B1;//-0.485;
    llc_cal.Freq = LLC_PWM_MAX_FREQ;
    llc_cal.Openloop_ssValue = LLC_PWM_MAX_FREQ;
    DemoState = IDLE;

    /* 10. HRTIM start-up Config */
    /* Stop HRTIM's outputs TA1, TA2, TB1 and TB2 */
    /* Start HRTIM's Master timer, TIMER A and B */
    llc_cal.Freq = LLC_PWM_MAX_FREQ;
    int CurrentPeriod = LLC_PWM_MAX_FREQ;
    ...
    HAL_HRTIM_WaveformOutputStop(&hhrtim,
                                 ...
                                 | HRTIM_OUTPUT_TB1
                                 | HRTIM_OUTPUT_TB2);
    if(HAL_UART_Receive_IT(&UartHandle, &UART_Cmd, 1) != HAL_OK)
    {
        Error_Handler();
    }
    ...
    while (1)
    {
        ...
        */
        uart_tx_message.Vout = llc_cal.Vout*22.5f;
        uart_tx_message.I = llc_cal.Ia*40;
        uart_tx_message.SysState = DemoState;
        uart_tx_message.OC = fg_OC;
        uart_tx_message.OV = fg_OV;
        uart_tx_message.CompF = fault_state;
        uart_tx_message.llc_freq = llc_cal.Freq;
        Debug_UART();

        /**
            Used for llc software protection
        */
        if(Vout >= OVP_VALUE)
        {
            debounceOV2 = 0;
        ...
        /**
        ** Testing for HRTIM fault event(Ia >OCP_VALUE)
        */
        fault_state = __HAL_HRTIM_GET_FLAG(&hhrtim, HRTIM_FLAG_FLT1);
        if(fault_state) {
        ...
        /* ---------------- */
        /* Fault management : any fault event occur ,shutdown output*/
        /* ---------------- */
        if(DemoState != FAULT)
        {
            if ((__HAL_HRTIM_GET_FLAG(&hhrtim, HRTIM_FLAG_FLT1) == SET)
                    || (__HAL_HRTIM_TIMER_GET_FLAG(&hhrtim, HRTIM_TIMERINDEX_TIMER_A, HRTIM_TIM_IT_DLYPRT))||fg_OC||fg_OV)
            {
                DemoState = FAULT;
...
static void GPIO_LLC_HB_outputs_Config(void)
{
    ...
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);  //ov


    GPIO_InitStruct.Pin = GPIO_PIN_5;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); //OC

    GPIO_InitStruct.Pin = GPIO_PIN_6;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET); //OC COMP
}
...
//    HRTIM_EventCfgTypeDef pEventCfg;
    ...
    /* ------------------------------------------------------------------------ */
    timebase_config.Period = LLC_PWM_MAX_FREQ;
    ...
    timebase_config.Period = (35 * LLC_PWM_MAX_FREQ)/100 + ADC_CONVERSION_TIME;
    ...
    timebase_config.Period = (85 * LLC_PWM_MAX_FREQ)/100 + ADC_CONVERSION_TIME;
    ...
    compare_config.CompareValue = (55 * LLC_PWM_MAX_FREQ)/100;     /* turn-on at 55% */
    ...
    /* Set compare registers for Turn-on on SR2 */
    compare_config.CompareValue = (95 * LLC_PWM_MAX_FREQ)/100;     /* turn-on at 95% */
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_MASTER,
                                    HRTIM_COMPAREUNIT_4,
                                    &compare_config);
    ...
    compare_config.CompareValue = LLC_PWM_MAX_FREQ + 1;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_MASTER,
                                    HRTIM_COMPAREUNIT_2,
                                    &compare_config);
    ...
    compare_config.CompareValue = (65 * LLC_PWM_MAX_FREQ)/100;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_MASTER,
                                    HRTIM_COMPAREUNIT_3,
                                    &compare_config);
    ...
    compare_config.CompareValue = (85 * LLC_PWM_MAX_FREQ)/100;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_MASTER,
                                    HRTIM_COMPAREUNIT_4,
                                    &compare_config);
    ...
    */
    ...
    compare_config.CompareValue = (15 * LLC_PWM_MAX_FREQ)/100;  /* Turn-on at 5% */
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_TIMER_B,
                                    HRTIM_COMPAREUNIT_1,
                                    &compare_config);
    ...
    compare_config.AutoDelayedMode = HRTIM_AUTODELAYEDMODE_REGULAR;
    compare_config.AutoDelayedTimeout = 0;
    compare_config.CompareValue = (35 * LLC_PWM_MAX_FREQ)/100;  /* Turn-off at 45% */
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_TIMER_B,
                                    HRTIM_COMPAREUNIT_2,
                                    &compare_config);
    ...
    compare_config.AutoDelayedMode = HRTIM_AUTODELAYEDMODE_REGULAR;
    compare_config.AutoDelayedTimeout = 0;
    compare_config.CompareValue = (15 * LLC_PWM_MAX_FREQ)/100 - ADC_CONVERSION_TIME;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_TIMER_B,
                                    HRTIM_COMPAREUNIT_3,
                                    &compare_config);
    ...
    compare_config.AutoDelayedMode = HRTIM_AUTODELAYEDMODE_REGULAR;
    compare_config.AutoDelayedTimeout = 0;
    compare_config.CompareValue = (65 * LLC_PWM_MAX_FREQ)/100 - ADC_CONVERSION_TIME;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
    ...
    compare_config.AutoDelayedMode = HRTIM_AUTODELAYEDMODE_REGULAR;
    compare_config.AutoDelayedTimeout = 0;
    compare_config.CompareValue = (85 * LLC_PWM_MAX_FREQ)/100 - ADC_CONVERSION_TIME;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_TIMER_C,
                                    HRTIM_COMPAREUNIT_4,
                                    &compare_config);
    ...
    /* ------------------------------------------- */
    adc_trigger_config.Trigger = HRTIM_ADCTRIGGEREVENT24_MASTER_CMP2
...
//    pEventCfg.FastMode = HRTIM_EVENTFASTMODE_DISABLE;
    ...
    /* ---------------------*/
    fault_config.Filter = HRTIM_FAULTFILTER_7;//HRTIM_FAULTFILTER_3;  //62.5ns 8/128mhz 8/128/4 = 250ns;
...
//
    ...
    /* ------------------------------------------------ */
    /* Compare 2 is used as ADC trigger for SR1 sampling before turn-off */
    compare_config.CompareValue = (35 * LLC_PWM_MAX_FREQ)/100 - ADC_CONVERSION_TIME;
    HAL_HRTIM_WaveformCompareConfig(&hhrtim,
                                    HRTIM_TIMERINDEX_MASTER,
                                    HRTIM_COMPAREUNIT_2,
                                    &compare_config);
...
void DAC1_CH2_Config(void)
{
    ...
    if (HAL_DAC_SetValue(&DacHandle, DAC_CHANNEL_2, DAC_ALIGN_12B_R, OCP_VALUE) != HAL_OK) //4FF =  1279 , 1279*3.3/4095 = 1.03V
    {
        /* Setting value Error */
        Error_Handler();
    }
...
{
    HAL_UART_Receive_IT(UartHandle, &UART_Cmd, 1);
    /* Set transmission flag: transfer complete */
    UartReady = SET;
...
static void SystemClock_Config(void)
{
    ...
    PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_HRTIM1|RCC_PERIPHCLK_USART1
                                         |RCC_PERIPHCLK_ADC12|RCC_PERIPHCLK_TIM1;
...
void LLC_Cal()
{
    llc_cal.Vdiff = llc_cal.Vref -llc_cal.Vout;
    llc_cal.Vloop = llc_cal.Vloop_last +(llc_cal.B0 *llc_cal.Vdiff)+(llc_cal.B1 *llc_cal.Vdiff_last);
    llc_cal.Vloop = (llc_cal.Vloop < 0) ?0 :llc_cal.Vloop;
    llc_cal.Vloop = (llc_cal.Vloop > 4) ? 4 :llc_cal.Vloop;
    llc_cal.Vdiff_last = llc_cal.Vdiff;
    llc_cal.Vloop_last = llc_cal.Vloop;
}
